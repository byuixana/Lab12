...
{
    return .5 * drag * density * (velocity * velocity) * (M_PI * (radius * radius));
}
...
inline double linearInterpolation(Mapping zero, Mapping one, double domain)
{
    return linearInterpolation(zero.domain, zero.range, one.domain, one.range, domain);
}
